...
public class SearchHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String nameSub = request.getParameter("cbSub");
        HttpSession session = request.getSession();
        accountDTO account = (accountDTO) session.getAttribute("ACCOUNT");
        try  {
            historyDAO dao = new historyDAO();
            ArrayList<historyDTO2> list = dao.getSearch(nameSub, account.getName());
            session.setAttribute("NAMESUBHISTORY", nameSub);
            session.setAttribute("LISTHIST", list);
            session.setAttribute("HISTORYPAGE", "");
